...
+ (void)initialize
{
    if (self == [GDIIndexBar class]) {
        ...
        if ([self isOS7OrLater]) {
            UIOffset offset = UIOffsetMake(10.5f, 0.f);
...
- (void)reload
{
    ...
    _lineHeight = [@"0" sizeWithAttributes:[NSDictionary dictionaryWithObject:self.textFont forKey:NSFontAttributeName]].height;
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i = 0; i < indexCount; i++) {
        ...
        CGSize textSize = [text sizeWithAttributes:[NSDictionary dictionaryWithObject:self.textFont forKey:NSFontAttributeName]];
        ...
        [text drawInRect:CGRectMake(shadowPoint.x, shadowPoint.y, textSize.width, _lineHeight)
          withAttributes:[NSDictionary dictionaryWithObject:self.textFont forKey:NSFontAttributeName]];
        
        // draw normal color
        [self.textColor set];
        [text drawInRect:CGRectMake(point.x, point.y, textSize.width, _lineHeight)
          withAttributes:[NSDictionary dictionaryWithObjectsAndKeys:self.textFont, NSFontAttributeName, self.textColor, NSForegroundColorAttributeName, nil]];
